...
CODES = {
    ...
    # Define codes for test sequence start/end
    0x98: "Testing Start",
    0x99: "Testing End",
    0x9A: "Test Mode Exiting"
}
...
def interpret_message(data):
    ...
    if len(data) == 1:
        ...
        if code in [0x93, 0x95, 0x97]:  # Collect errors for summary
            if data_value != 0:
    ...
    elif code in [0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0xA0, 0xA1, 0xA2, 0xA3, 0xA4, 0xA5, 0xA6, 0xA7, 0xA9, 0xAA]:
    ...
    elif code in [0x98, 0x99, 0x9A]:
        return Fore.CYAN + f"INFO: {message}"
    else:
    ...
    try:
        with serial.Serial(serial_port, 115200) as ser:
            ...
            while True:
                ...
                if len(line) == 2:
                    code = int(line, 16)
                    message = CODES.get(code, f"Unknown code: {line}")
                    print(interpret_message([code]))
                elif len(line) == 14:
                    code = int(line[:2], 16)
                    data_value = int(line[2:], 16)
                    message = f"{CODES.get(code, f'Unknown code: {line[:2]}')}: {data_value}"
                    if code in [0x93, 0x95, 0x97]:  # Collect errors for summary
                        if data_value != 0:
                            errors.append(message)
                    print(interpret_message([code] + list(line[2:])))
                if code == 0x9A:
                    print_summary()
                    break
...
def print_summary():
    print(Fore.CYAN + "\nTest Mode Exiting")
    print(Fore.CYAN + "Summary of Errors:")
    if not errors:
        print(Fore.GREEN + "No errors detected.")
    else:
        for error in errors:
